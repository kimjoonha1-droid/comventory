    ...
    <div id="page-content-wrapper">
        <div class="container p-4">
			    ...
			    <c:if test="${board.boardRefLvl == 1}">
			        <div class="card mb-2 border-primary">
			            <div class="card-header bg-primary text-white fw-bold">
			                <i class="bi bi-chat-dots me-1"></i> 답변 - COMVENTORY
			            </div>
			            <div class="card-body">
			                <div class="row mb-0.5">
			                    <div class="col-md-3 fw-bold">등록일</div>
			                ...
			                <c:forEach var="reply" items="${boardList}">
			                    <c:if test="${reply.boardRefLvl == 2 && reply.boardRef == board.boardRef}">
			                        <div class="card mb-3 mt-3 border-primary ms-4">
			                            <div class="card-header bg-primary text-white fw-bold">
			                                <c:choose>
			                                    <c:when test="${reply.userAccess == 800 || reply.userAccess == 900}">
			                                        <i class="bi bi-chat-dots me-1"></i> 대댓글 - ${reply.empName}
			                                    </c:when>
			                                    <c:otherwise>
			                                        <i class="bi bi-chat-dots me-1"></i> 대댓글 - COMVENTORY
			                                    </c:otherwise>
			                                </c:choose>
			                            </div>
			                            <div class="card-body">
			                ...
			                <!-- 대댓글 버튼 -->
			                <c:if test="${loginUser != null && (loginUser.user_access != 800 && loginUser.user_access != 900 || loginUser.emp_no == writerEmpNo)}">
            ...
            <c:if test="${loginUser != null && loginUser.user_access != 800 && loginUser.user_access != 900}">
			    <div class="card mb-4 border-secondary">
                    ...
                    <div class="card-body">
                        <c:forEach var="board" items="${boardList}">
                            <c:if test="${board.boardRefLvl == 0}">
                                <form action="${pageContext.request.contextPath}/board/answer" method="post">
                                    <input type="hidden" name="boardRef" value="${board.boardRef}">
            ...
            <div class="text-start">
                <a href="${pageContext.request.contextPath}/board/list?currentPage=${param.currentPage}&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}&answerFilter=${param.answerFilter}" class="btn btn-secondary">
